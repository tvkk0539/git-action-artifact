...
# Apply Overrides
if [ -n "$OVERRIDE_ACCOUNT_3" ]; then
    echo "Using Hardcoded Account 3 Override"
    ACCOUNT_3="$OVERRIDE_ACCOUNT_3"
fi
...
# 3. SESSION_RESTORE_URL_THREE (Secret)
...
if [ -n "$HARDCODED_RESTORE_URL_DISCORD" ]; then
    ...
    RESTORE_MODE="nested"
elif [ -n "$SESSION_RESTORE_URL_THREE" ]; then
